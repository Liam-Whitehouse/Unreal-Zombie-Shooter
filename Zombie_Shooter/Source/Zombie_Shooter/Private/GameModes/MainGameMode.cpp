...
DEFINE_LOG_CATEGORY(LogZombieShooterGameMode)
...
void AMainGameMode::InitGameLift()
{
	UE_LOG(LogZombieShooterGameMode, Log, TEXT("Initializing the Gamelift Server"));
	...
	auto onGameSession = [=](Aws::GameLift::Server::Model::GameSession gameSession)
		{
			...
			UE_LOG(LogZombieShooterGameMode, Log, TEXT("GameSession Initializing: %s"), *gameSessionId);
	...
	auto onProcessTerminate = [=]()
		{
			UE_LOG(LogZombieShooterGameMode, Log, TEXT("Game Game Server Process is terminating"));
	...
	auto onHealthCheck = [=]()
		{
			UE_LOG(LogZombieShooterGameMode, Log, TEXT("Performing Health Check."));
			return true;
		};
	...
	UE_LOG(LogZombieShooterGameMode, Log, TEXT("Calling Process Ready"));
	GameLiftSDKModule->ProcessReady(ProcessParameters);
}

void AMainGameMode::SetServerParameters(FServerParameters& OutParams)
{
	...
	{
		UE_LOG(LogZombieShooterGameMode, Log, TEXT("AUTH_TOKEN: %s"), *OutParams.m_authToken);
	}

	//The Host/compute-name of the GameLift Anywhere Instance
	if (FParse::Value(FCommandLine::Get(), TEXT("-hostid="), OutParams.m_hostId))
	{
		UE_LOG(LogZombieShooterGameMode, Log, TEXT("HOST_ID: %s"), *OutParams.m_authToken);
	}

	//The Anywhere Fleet ID
	if (FParse::Value(FCommandLine::Get(), TEXT("-fleetid="), OutParams.m_fleetId))
	{
		UE_LOG(LogZombieShooterGameMode, Log, TEXT("FLEET_ID: %s"), *OutParams.m_fleetId);
	}

	//The WebSocket URL (GameLiftServiceSdkEndpoint).
	if (FParse::Value(FCommandLine::Get(), TEXT("-websocketurl="), OutParams.m_webSocketUrl))
	{
		UE_LOG(LogZombieShooterGameMode, Log, TEXT("WEBSOCKET_URL: %s"), *OutParams.m_webSocketUrl);
	}

	//The PID of the Running Process
	OutParams.m_processId = FString::Printf(TEXT("%d"), GetCurrentProcessId());
	UE_LOG(LogZombieShooterGameMode, Log, TEXT("PID: %s"), *OutParams.m_processId);
}

void AMainGameMode::ParseCommandLinePort(int32& OutPort)
{
